...
import { calculateAvailableWithdrawalBalance, isDriverWalletBlocked } from "./driver-finance";
...
describe("driver withdrawal balance", () => {
  ...
  it("blocks a driver at the configured -100 جنيه credit limit", () => {
    expect(isDriverWalletBlocked(-100, 100)).toBe(true);
    expect(isDriverWalletBlocked(-99, 100)).toBe(false);
    expect(isDriverWalletBlocked(20, 100)).toBe(false);
  });
